...
public class JavTestAPIMain {
	...
	public void createIndex() {
		...
		for (int i = 0; i < 10; i++) {
			...
			article.setPostDate("sdsad");
			article.setTitle("asdsAS");
			article.setTags("asdaS");
			articles.add(article);
		}
		//List<Article> articles = dbIntegration.getArticles();
		this.loadRecords(articles);
	}
	
	
	public void loadRecords(List<Article> articles) {
		Client client = getClient();
	    for (Article article : articles) {
	    	 String [] ar = new String[1]; 
		     ar[0] = article.getTags();
	    	 client.prepareIndex("shah1_test","shah1_type", article.getId()).setSource(putJsonDocument(article)).execute().actionGet();
		}   
	}
